    ...
    <div v-if="profileImage">
      ...
      <div class="relative block relative h-40 w-40 mx-auto text-gray-500  rounded-full">
        <div v-if="profileImage" class="flex items-center justify-center w-full h-full bg-gray-300 rounded-full shadow-md">
          <div v-if="typeof profileImage === 'boolean'">
            <div class="h-40 w-40 rounded-full bg-customGreen flex justify-center items-center text-white text-5xl tracking-widest cursor-pointer">
              MH
            </div>
          </div>
          <img v-if="typeof profileImage === 'string'" draggable="false" :src="profileImage" class="h-full w-full object-cover rounded-full" alt=""/>
        </div>
        ...
        <div v-click-outside="closeProfileImageSettings" v-if="profileImageSettings" class="absolute h-20 w-28 rounded-lg border bg-white -right-10 -bottom-[73px]">
          <ul class="[&>li]:text-center [&>li]:py-1.5 [&>li]:cursor-pointer">
            <li class="border-b">
              <label for="imageUpdate" class="block cursor-pointer">
                Edit
        ...
        <div v-if="!editable" class="absolute inset-0 h-full w-full rounded-full bg-gray-100 opacity-[0.5]"></div>
      </div>
    </div>
...
function setProfileImage(event) {
  if (event?.target?.files[0]) {
    ...
    profileImageSettings.value = false;
  }
}

function closeProfileImageSettings(event) {
  if (event.target) profileImageSettings.value = false;
}
